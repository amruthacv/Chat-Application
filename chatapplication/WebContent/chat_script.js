var wsUri = getServerRootUri() + "/chatapplication/chat-server";
var currentUsername = "";
var chatWebsocket;
...
function signin() {
    ...
    if (retVal != "" && retVal != null) {
        ...
        chatWebsocket = new WebSocket(wsUri, "chatapplication");
...
function button_sendMessage() {
    ...
    if(chatMessageTextID.value==''){
      	 alert("Please enter the message!");
       }
    //var recipient = recipientTextID.value;
    
    //if(re=="gc")
    	//recipientTextID.value = "all";
   
    if(chatMessageTextID.value!=''){
    	var re = prompt("Recipient name / Enter 'gc' for group chat : ", "user-name", "");
    if (chatString.length > 0) {
       chatWebsocket.send("ctmsg"  + currentUsername  + "~" + chatString + "~" +   re );
        //chatWebsocket.send("ctmsg" + currentUsername + ":" + chatString +  ":" +re);
        chatMessageTextID.value = "";
        //recipientTextID.value = "";
      }
    }
    
   
}
/*function button_groupChat() {
	recipientTextID.value = "all";
	
	//recipientTextID1.value=re;
	}*/
